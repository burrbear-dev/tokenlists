 ...
 */
async function downloadAssets(tokens, force = false, options = {}) {
  ...
  for (const token of tokens) {
    ...
    if (options.verbose) {
      log(
        ...
        } - Original: ${originalFilename}, Extension: ${fileExtension}, Target: ${filename}`
      )
    }
    ...
    if (!force && fs.existsSync(targetPath)) {
      ...
      log(`File does not exist, will download: ${filename}`)
    }
 ...
 */
async function integrateTokenList(options = {}) {
  ...
  try {
    ...
    await downloadAssets(validTokens, options.force, options)
...
async function init() {
  ...
  await integrateTokenList(options)
}
